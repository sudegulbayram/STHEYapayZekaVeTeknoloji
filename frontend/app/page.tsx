...
export default function Home() {
  ...
  const handleSendMessage = async () => {
    ...
    addLog("İstek backend'e gönderiliyor...", "info");

    try {
      if (currentFile) {
        addLog("Dosya FastAPI sunucusuna yükleniyor...", "loading");
        ...
        if (response.ok) {
          ...
          addLog("Dosya başarıyla işlendi.", "success");
          
          const aiResponseText = data.message || "İşlem başarılı";
          ...
          addLog("Hata: " + (errorData.detail || "İşlem başarısız"), "info");
          setChat(prev => [...prev, { sender: "ai", text: `Yükleme başarısız oldu: ${errorData.detail || "Sunucu hata döndü"}` }]);
        }
      } else {
        addLog("Sadece metin girildi, dosya bekleniyor.", "info");
        setChat(prev => [...prev, { 
          sender: "ai", 
          text: "Şu anki altyapımızda analiz için bir sözleşme belgesi veya resmi yüklemeniz gerekmektedir. Lütfen ataç ikonundan bir dosya seçin." 
        }]);
      }
    } catch (error) {
      addLog("Bağlantı hatası: FastAPI kapalı olabilir.", "info");
      setChat(prev => [...prev, { 
        sender: "ai", 
        text: "Arka uç (backend) sunucusuna ulaşılamadı. Uvicorn'un çalıştığından emin olun." 
      }]);
    } finally {
      setIsTyping(false);
    }
